...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE carbon_data ADD COLUMN amount numeric;
  END IF;
...
-- Update amount column to use total_emissions if amount is null
UPDATE carbon_data 
SET amount = total_emissions 
WHERE amount IS NULL AND total_emissions IS NOT NULL;

-- Add constraint to amount column
